...
def post_message(content, tries=0):
    ...
    if tries > 5:
        ...
        return False
    response = requests.post(url=HOOK_URL, json=msg)
...
def lambda_handler(event, context):
    ...
    if len(new_messages) >= 60:
        response = requests.post(url=HOOK_URL, json={"text": "Too many new messages... Check the site: https://aws.amazon.com/new. "})
        print(response)
        return
